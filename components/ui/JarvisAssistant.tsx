...
const RESPONSES: Record<string, { text: string; nav?: string }> = {
  ...
  trading:  { text: "The All-Season Bot is our flagship — a fully autonomous crypto rebalancer with market regime detection. 4 phases, 5-stage pipeline, +194.79% backtested. Want me to take you there?", nav: '/trading' },
  bot:      { text: "The trading bot classifies market conditions (BTC/ETH/ALTCOIN phases + RISK_OFF), generates allocation proposals, passes them through a risk gate, then executes. Backtested +194.79% over 12 months vs buy-and-hold -6%.", nav: '/trading' },
...
export default function JarvisAssistant() {
  ...
  // Hide on /starmap (+/dk/starmap) so the KCP-90 terminal at bottom-right does
  ...
  return (
    <>
      {/* CommBubble Hotfix (Apr 26 2026) — Jarvis sits bottom-right while
          UniverseChat keeps bottom-left, eliminating the z-[60] vs z-50 overlap
          observed on /cards, /shop, /break-room, /wallet, /quantum/chat,
          /about, /home and /cards/alpha/deck-builder. The earlier Sprint 16
          Task 6 left-side workaround for the KCP-90 terminal is preserved by
          the route-skip above (returns null on /starmap and /dk/starmap). */}
      ...
      <AnimatePresence>
        {open && !minimized && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-[60] w-80 sm:w-96 rounded-2xl overflow-hidden"
            style={{
